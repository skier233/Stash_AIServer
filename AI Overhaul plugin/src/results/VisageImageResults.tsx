...
interface VisageImageResultsProps {
  ...
  onClose: () => void;
  React: any;
  directImageUrl?: string; // Direct image URL for batch images
}
...
const VisageImageResults: React.FC<VisageImageResultsProps> = ({ 
  ...
  React.useEffect(() => {
    const resolveImageUrl = async () => {
      // If directImageUrl is provided (for batch images), use it directly
      if (directImageUrl) {
        console.log('VisageImageResults: Using direct image URL for batch image:', directImageUrl.substring(0, 50) + '...');
        setImageUrl(directImageUrl);
        setImageId(task?.input_data?.entity_id || 'batch_image');
        setImageError(null);
        return;
      }
      ...
      React.createElement('div', {
        ...
        React.createElement('div', { key: 'title-section' }, [
          ...
          React.createElement('div', {
            ...
            // Add link to view image in Stash if we have imageId and it's not a batch image
            imageId && !directImageUrl ? React.createElement('a', {
              key: 'image-link',
              href: `/images/${imageId}`,
              target: '_blank',
