(function () {
  ...
  const AIOverhaulSettingsPage = () => {
    ...
    const viewJobResults = async (job) => {
      try {
        console.log('🔍 Viewing batch results for job:', job);
        
        // Fetch detailed job info with tasks
        const jobDetailUrl = `http://${serverSettings.stashAIServer}:${serverSettings.port}/api/queue/job/${job.job_id}`;
        ...
        if (response.ok) {
          ...
          } else if (tasks.length === 1 && tasks[0].output_json) {
            // Single task - use individual results
            viewTaskResults(tasks[0]);
          } else {
            alert('No viewable results found for this job');
          }
        }
      } catch (error) {
        console.error('Failed to view job results:', error);
    ...
    const cancelTask = async (task) => {
      try {
        ...
        const cancelUrl = `http://${serverSettings.stashAIServer}:${serverSettings.port}/api/queue/cancel/${task.task_id}`;
        ...
        if (response.ok) {
          alert(`Task ${task.task_id.substring(0, 8)}... cancelled successfully`);
          // Refresh the history
            ...
            React.createElement('tbody', {
              ...
              jobRows.push(React.createElement('tr', {
                ...
                React.createElement('td', {
                  ...
                  // Batch view button for multi-task jobs with results
                  (job.tasks && job.tasks.length > 1 && 
                   job.tasks.some(task => task.output_json) && 
                   (job.status === 'completed' || job.status === 'partial')) && React.createElement(Button, {
                    key: 'batch-view-btn',
                    variant: 'outline-info',
                    size: 'sm',
                    onClick: (e) => { e.stopPropagation(); viewJobResults(job); },
                    title: 'View batch results'
                  }, React.createElement('i', { className: 'fas fa-images' })),
                  React.createElement('small', {
                    key: 'expand-hint',
                    className: 'text-muted',
                    style: { marginLeft: job.tasks && job.tasks.length > 1 && job.tasks.some(task => task.output_json) ? '8px' : '0' }
                ...
                job.tasks.forEach((task, taskIndex) => {
                  jobRows.push(React.createElement('tr', {
                    ...
                    React.createElement('td', {
                      ...
                      (task.status === 'pending' || task.status === 'running') && React.createElement(Button, {
                        key: 'task-cancel-btn',
                        variant: 'outline-danger',
                        size: 'sm',
                        onClick: (e) => { e.stopPropagation(); cancelTask(task); },
                        title: 'Cancel task'
